...
namespace BoxeDeCria.Scout
{
    ...
    {
        public static EvidenceAudit Audit(ScoutDossierDto d, float minimumCoverage = 0.90f)
        {
            ...
            var coverage = claims.Count == 0 ? 0f : (float)supported / claims.Count;
            var reasons = new List<string>();

            if (d.status != "approved") reasons.Add("Status do dossiê não é approved.");
            if (claims.Count == 0) reasons.Add("Dossiê sem claims auditáveis.");
            ...
            {
                if (claim.confidence < 0 || claim.confidence > 1) reasons.Add($"Claim {claim.id}: confidence fora de 0–1.");
                if (claim.status == "supported" && (claim.evidence == null || claim.evidence.Count == 0)) reasons.Add($"Claim {claim.id}: supported sem evidência.");
                if (claim.evidence != null && claim.evidence.Any(e => e == null || !sourceIds.Contains(e.sourceId))) reasons.Add($"Claim {claim.id}: referência de fonte ausente.");
            }
            ...
            return new EvidenceAudit(reasons.Count == 0, coverage, supported, uncertain, unsupported, conflicted, reasons.Distinct().ToArray());
        }
    }
}
